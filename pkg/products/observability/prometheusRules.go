	...
	return &resources.AlertReconcilerImpl{
		...
		Alerts: []resources.AlertConfiguration{
...

			{
				...
				Rules: []monitoringv1.Rule{
					...
					},
					{
						...
						Expr:   intstr.FromString(fmt.Sprintf("((sum by(persistentvolumeclaim, namespace) (kubelet_volume_stats_used_bytes{namespace=~'%s.*'})) / (sum by(persistentvolumeclaim, namespace) (kube_persistentvolumeclaim_resource_requests_storage_bytes{namespace=~'%s.*'}))) * 100 > 85", nsPrefix, nsPrefix)),
						For:    "15m",
						Labels: map[string]string{"severity": "warning", "product": installationName},
					},
					{
					...
					},
					{
						...
						Expr:   intstr.FromString(fmt.Sprintf("(predict_linear(kubelet_volume_stats_available_bytes{job='kubelet', namespace=~'%s.*'} [6h], 4 * 24 * 3600) <= 0 and kubelet_volume_stats_available_bytes{job='kubelet', namespace=~'%s.*'}  / kubelet_volume_stats_capacity_bytes{job='kubelet'} < 0.15)", nsPrefix, nsPrefix)),
						For:    "15m",
						Labels: map[string]string{"severity": "warning", "product": installationName},
					},
					{
						...
						Expr:   intstr.FromString(fmt.Sprintf("(kubelet_volume_stats_available_bytes{job='kubelet', namespace=~'%s.*'} / kubelet_volume_stats_capacity_bytes{job='kubelet', namespace=~'%s.*'} < 0.03)", nsPrefix, nsPrefix)),
						For:    "15m",
						Labels: map[string]string{"severity": "warning", "product": installationName},
					},
...

			{
				...
				Namespace: r.Config.GetOperatorNamespace(),
				GroupName: "observability-operator-endpoint.rules",
				Rules: []monitoringv1.Rule{
					{
				...
				Rules: []monitoringv1.Rule{
					{
						...
						Expr:   intstr.FromString(fmt.Sprintf("csv_abnormal{phase=~'Pending|Failed',exported_namespace=~'%s.*'}", nsPrefix)),
						For:    "15m",
						Labels: map[string]string{"severity": "warning", "product": installationName},
					},
				},
